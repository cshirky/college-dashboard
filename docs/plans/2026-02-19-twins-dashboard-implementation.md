# School Twins Dashboard Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a third dashboard page where users paste school names/UNITIDs and see the three closest "twin" institutions for each.

**Architecture:** All computation in-browser using JavaScript. Build CIP degree-count vectors per school, compute cosine similarity + enrollment distance for candidates matching on sector and locale. New Observable Framework page at `src/twins.md`.

**Tech Stack:** Observable Framework, D3, Observable Inputs, Observable Plot

---

### Task 1: Add twins page to Observable config and navigation
...
**Step 1: Add twins page to config**

In `observablehq.config.js`, add the twins page to the pages array:
...
**Step 2: Add navigation link to explorer page**

In `src/index.md`, after the existing compare link, add:

```markdown
<a href="/twins">→ Find school twins</a>
```

**Step 3: Add navigation link to compare page**

In `src/compare.md`, after the "← Back to Explorer" link, add:
...
```bash
git add observablehq.config.js src/index.md src/compare.md
git commit -m "feat: add twins page to config and navigation"
```

---

### Task 2: Create twins page — data loading and input

**Files:**
- Create: `src/twins.md`

**Step 1: Create the twins page with data loading, helper functions, and input textarea**

Create `src/twins.md` with the following content:

````markdown
<h1>School Twins</h1>

<a href="/">← Back to Explorer</a>
<a href="/compare" style="margin-left: 1rem;">→ Compare categories</a>
...
Navigate to: `http://127.0.0.1:3000/twins`
Expected: Page loads with textarea input, Find Twins button

**Step 3: Commit**

```bash
git add src/twins.md
git commit -m "feat: add school twins page with similarity matching"
```
...
Expected: Build succeeds with 3 pages (index, compare, twins), all links validated
